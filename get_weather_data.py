...
processed_locations = set([int(os.path.basename(f).split('_')[1].split('.')[0]) for f in existing_checkpoints])
...
for i in range(0, len(locs), BATCH_SIZE):
    ...
    print(f"Processing batch {i//BATCH_SIZE + 1}/{-(-len(locs)//BATCH_SIZE)}, locations {i} to {min(i+BATCH_SIZE-1, len(locs)-1)}")
    ...
    for j, response in enumerate(batch_responses):
        ...
        hourly_df["location_id"] = loc_idx  # Add location ID for reference
        ...
        merged_df = pd.merge(hourly_df, daily_df[["date_only", "sunrise", "sunset"]], 
        ...
        # Save batch checkpoint every 5 locations
        if len(all_hourly_dfs) % 5 == 0:
...
print(f'Final hourly dataframe contains {len(final_hourly_df)} rows from {len(all_hourly_dfs)} locations')

# Save to parquet
final_hourly_df.to_parquet('data/final/weather_hourly_all_locations.parquet', index=False, compression='brotli')
